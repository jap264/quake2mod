...
void ChasecamTrack (edict_t *ent)
{
        ...
        if (VectorCompare(ent->movedir, ent->s.origin))
	{
        ...
        if (ent->chasedist2 > 3)
       	{
                if (ent->owner->client->oldplayer->client)
                        free(ent->owner->client->oldplayer->client); 
...
void CheckChasecam_Viewent (edict_t *ent)
{
        ...
        Create a fake client for the old player just in case its needed
        */
        if (!ent->client->oldplayer->client)
        {
	...
	if ((ent->client->chasetoggle == 1) && (ent->client->oldplayer))
        {
                ...
                ent->client->oldplayer->client->ps = ent->client->ps;
                VectorCopy(ent->client->v_angle, ent->client->oldplayer->client->v_angle);
                VectorCopy(ent->client->resp.cmd_angles, ent->client->oldplayer->client->resp.cmd_angles);
